    ...
    *normalize* :
    ...
    def multiply_factor(self, other_factor):
        ...
        *other_factor* : a different Factor object
        ...
        psi = np.zeros(np.product(list(phi1.card.values())))
        # print (psi)
        for i in range(len(psi)):
            psi[int(i)] = phi1.cpt[int(j)]*phi2.cpt[int(k)]
            # print(i, "ith")
            for rv in rv_order:
                assignment[rv] += 1
    ...
    def reduce_factor(self, rv, val):
        ...
        *val* : a string
        ...
        val_idx = self.bn.F[rv]['values'].index(val) #can judge if the value is not in cpt
        # print(val_idx)
        rv_card = self.card[rv]
        rv_stride = self.stride[rv]
